...
class Aviacao{
    ...
    public function recuperarDados(){
        $conexao = new Conexao();
        $sql = "select * from aviacao order by nome asc";
    ...
    public function inserir($dados, $img){
        ...
        $sql = "insert into aviacao (nome, marca, cor, motor, placa, renavam, chassi, valor, ano, foto) values ('$nome', '$marca', '$cor', '$motor', '$placa', '$renavam', '$chassi', '$valor', '$ano', '$foto')";

        /*echo $sql; die; MOSTRAR O SQL*/
        return $conexao->executar($sql);
    }

    public function excluir($idavicao){

        $conexao = new Conexao();

        $sql = "delete from aviacao where idaviacao = $idavicao;";
    ...
    public function uploadFoto($img)
    {
        ...
        if ($img['foto']['error'] == UPLOAD_ERR_OK) {
            ...
            $destino = '../upload/aviacao/' . $img['foto']['name'];

            move_uploaded_file($origem, $destino);
        }
    }

    public function carregarPorId($idaviacao){
        $conexao = new Conexao();

        $sql = "select * from aviacao where idaviacao = $idaviacao";
    ...
    public function alterar($dados){
        ...
        $sql = "update aviacao set nome = '$nome', marca='$marca', cor='$cor', motor='$motor', placa='$placa', renavam='$renavam', chassi='$chassi', valor='$valor', ano='$ano', foto='$foto' 
                where idaviacao = $idaviacao";
